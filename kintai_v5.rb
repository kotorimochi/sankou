...
# ハッシュ化（シークレットとbodyから署名を作る）（バイト列。コンピュータが扱う生のデータの並び）
require "openssl"

# Base64.strict_encode64（エンコード）を、読める文字列に変換する
require "base64"
...
def kintone_put(record_id, record)
  ...
  # ENV[...] が「取り出す」書き方なので、「"KINTAI_TOKEN"」は文字列として送られない
  req["X-Cybozu-API-Token"] = ENV["KINTAI_TOKEN"]
  req["Content-Type"] = "application/json"
  req.body = {
    app: 1,
    id: record_id,
...
def line_post(message, reply_token)
  ...
  # 外側がダブルクォートなので、中で同じダブルクォートを使うと、そこで文字列が終わったと判断される
  req["Authorization"] = "Bearer #{ENV['LINE_TOKEN']}"
  req["Content-Type"] = "application/json"
  req.body = {
...
post "/webhook" do
  ...
  # 「ENV」と「request.env」は別物
  # LINEが送ってきたヘッダー
  signature = request.env["HTTP_X_LINE_SIGNATURE"]

  ## OpenSSLの.digestで3つを変換する。「SHA256」（文字列）、「LINE_SECRET」、「body」
  hash = OpenSSL::HMAC.digest("SHA256", ENV["LINE_SECRET"], body)

  ## ハッシュをエンコードする
  my_signature = Base64.strict_encode64(hash)

if signature != my_signature

  # haltが実行されたら下の行は実行されない(elseの代用)
  halt 400, "署名が一致しません"
end
  ...
  if data["events"].nil? || data["events"].empty?
    halt 200
  end
  ...
  record = {
    ...
    kintone_post(2, ENV["MASTER_TOKEN"], record)
    ...
    result = kintone_get(2, ENV["MASTER_TOKEN"], query)
...
result = kintone_get(1, ENV["KINTAI_TOKEN"], query)
...
if result["records"].empty?
  ...
  record = {
    ...
    kintone_post(1, ENV["KINTAI_TOKEN"], record)
